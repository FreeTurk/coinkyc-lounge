...
export default function Entry({ title, image, article }) {
    const [isExp, setExp] = useState(true)
    return (
        <div className={(isExp ? "w-[320px] h-[320px] cursor-pointer" : "h-infoh w-5/6 xl:w-infow mt-0 absolute z-30") + ' flex m-16 bg-slate-200 rounded-2xl border-blue-300 border-2 drop-shadow-2xl transition-all ease-in-out duration-500'} onClick={() => {setExp(!isExp);document.getElementById("right").scrollTop;}
        }>
            <div className='p-8 w-full'>
                
                <div className={(isExp ? "text-xl flex-row" : "flex-col md:flex-row justify-center gap-8") + ' transition-all flex flex-row justify-between h-fit w-full items-center'}>
